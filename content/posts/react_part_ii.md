...
1. Effects triggered on state: Instead of triggering a side effect when something happens (an 'action'), trigger them based on certain values in the state.
...
1.  Some state, such as current url, the window size, the scroll position is going to live on the `window` object, so it won't be part of your state object. (unless you do something to keep them in sync, which seems painful.)
...
1. Actions and action creators seems like an unnecessary level of indirection.
1. How the state get's turned into the view. Redux has the [`useSelector`](https://react-redux.js.org/api/hooks#useselector) hook, or [`connect`](https://react-redux.js.org/api/connect). There's also questions on [where](https://redux.js.org/faq/react-redux#should-i-only-connect-my-top-component-or-can-i-connect-multiple-components-in-my-tree) to connect components to the state.
1. Everything about [effects](https://redux.js.org/usage/side-effects-approaches).
1. More generally, the core idea of using a reducer and immutable data is dead simple, yet somehow the redux tutorial is _huge_.
...
[Note: we likely won't use `unsubscribe` so there's a bit of over-engineering here.]
...
of this approach is that with Redux the actions are serializable.
...
1. `Map` and `Set` have a better definition of equals than the javascript versions. (e.g. when an object is a key in a `Map` referential equality probably is not what you want.)
1. 'Deep' methods. e.g. [`updateIn`](https://immutable-js.com/docs/v4.3.0/Map/#updateIn()).

Things I don't like amount immutable.js:
1. Boilerplate when defining types.
...
OK MPD 0.23.5
> idle
! (Add song from other app)
changed: playlist
OK
> idle
! (Play song from other app)
changed: player
changed: mixer
OK

```
Note that it leaves the `idle` state once it reports an event, which is why I needed to send it twice to be notified of the two actions in the other app.
...
`playlistinfo` gives the current queue. Notice there isn't a delimiter between the tracks but rather a new track seems to always start with a `file` attribute. `file` is likely the best way to globally identify a track.
...
That should give the basics for building an app, there's lots more commands available in the API which we will consult as needed. Next post we will do some up front design and figure out what MPD commands go with the features we want to implement.
